...
const useFilterSearch = () => {
  ...
  const [currentFiltered, setCurrentFiltered] = useState<any[]>(dummyMentors);

  // Function which takes a) an array of objects (data) and b) a searchValue as parameters. 
  // In the end, sets the currentFiltered state to the data filtered by what is entered in SearchForm.
  const filterSearch = (data: Array<Data>, searchValue: string) => {
    let filteredData: Array<Data> = data
      .map((item) => {
        const { name, id } = item;
        return { name: name.toLowerCase(), id: id };
      })
      .filter((item) => {
        if (item) {
    ...
    setCurrentFiltered(filteredData);
  };
