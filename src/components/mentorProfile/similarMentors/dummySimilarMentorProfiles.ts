
import hiba from '../../../../public/assets/hiba.png'

export const dummySimilarMentorProfiles = [
    {
      name: 'Hiba Badran',
      job: 'Founder of Empowered Futures',
      avatar: hiba,
      location: 'Calgary, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['Entrepeneurship', 'Management', 'Problem Solving'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 90 }, 
        {name: 'Adobe XD', percentage: 20 },
        {name: 'Networking', percentage: 40 },
        {name: 'Mindset Coaching', percentage: 70 },
        {name: 'Figma', percentage: 100 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Every day after 1 pm',
      
    },

    {
      name: 'Matt Engerer',
      job: 'Makes websites',
      avatar: hiba,
      location: 'Toronto, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['Computer', 'Management', 'Problem Solving'],
      percentBarSkills: [
        {name: 'React', percentage: 100 }, 
        {name: 'JS', percentage: 100 },
        {name: 'Networking', percentage: 20 },
        {name: 'Sitting', percentage: 90 },
        {name: 'Walking', percentage: 80 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 11 pm',
      

    },

    {
      name: 'Jason Bustamante',
      job: 'Coder',
      avatar: hiba,
      location: 'Toronto, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['Entrepeneurship', 'Management', 'Problem Solving'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 90 }, 
        {name: 'Adobe XD', percentage: 20 },
        {name: 'Networking', percentage: 40 },
        {name: 'Mindset Coaching', percentage: 70 },
        {name: 'Figma', percentage: 100 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 7 pm',
      

    },
    {
      name: 'Kelsey Hall',
      job: 'Coder',
      avatar: hiba,
      location: 'Toronto, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['Sprint', 'Leader', 'Code'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 50 }, 
        {name: 'Adobe XD', percentage: 100 },
        {name: 'Networking', percentage: 40 },
        {name: 'Mindset Coaching', percentage: 70 },
        {name: 'Figma', percentage: 10 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 7 pm',
      

    },
    {
      name: 'Joshua Padron-Uy',
      job: 'React Coder',
      avatar: hiba,
      location: 'Toronto, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['REACT', 'Management', 'Problem Solving'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 20 }, 
        {name: 'Adobe XD', percentage: 20 },
        {name: 'Networking', percentage: 100 },
        {name: 'Mindset Coaching', percentage: 70 },
        {name: 'Figma', percentage: 100 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 7 pm',
      

    },
    {
      name: 'Roland Mackintosh',
      job: 'Coder',
      avatar: hiba,
      location: 'Vancouver, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['Entrepeneurship', 'Management', 'Problem Solving'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 90 }, 
        {name: 'Adobe XD', percentage: 100 },
        {name: 'Networking', percentage: 40 },
        {name: 'Mindset Coaching', percentage: 30 },
        {name: 'Figma', percentage: 90 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 7 pm',
      

    },
    {
      name: 'Jamie Pantling',
      job: 'Codes',
      avatar: hiba,
      location: 'Toronto, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['Reacting', 'Management', 'Problem Solving'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 40 }, 
        {name: 'Adobe XD', percentage: 90 },
        {name: 'Networking', percentage: 100 },
        {name: 'Mindset Coaching', percentage: 70 },
        {name: 'Figma', percentage: 100 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 7 pm',
      

    },
    {
      name: 'Vincent Vuong',
      job: 'Codes',
      avatar: hiba,
      location: 'Toronto, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['Reacting', 'Management', 'Problem Solving'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 20 }, 
        {name: 'Adobe XD', percentage: 100 },
        {name: 'Networking', percentage: 40 },
        {name: 'Mindset Coaching', percentage: 70 },
        {name: 'Figma', percentage: 100 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 7 pm',
      

    },
    {
      name: 'Shiho Nagano',
      job: 'Codes',
      avatar: hiba,
      location: 'Toronto, Canada',
      responseTime: 'Usually responds within 1 day',
      skills: ['React', 'Code', 'Problem Solving'],
      percentBarSkills: [
        {name: 'Grow a Business', percentage: 20 }, 
        {name: 'Adobe XD', percentage: 100 },
        {name: 'Networking', percentage: 40 },
        {name: 'Mindset Coaching', percentage: 70 },
        {name: 'Figma', percentage: 100 },
      ],
      about: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aperiam aut voluptatibus, corrupti in placeat libero necessitatibus numquam architecto corporis? Quo nulla nisi aperiam maiores explicabo inventore ex quaerat voluptates dolor?',
      availability: 'Weekends after 7 pm',
      

    }
  
  ]