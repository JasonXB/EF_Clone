...
export default function OptionForm(props: any) {
  ...
  return (
    <React.Fragment>
      <label htmlFor={props.name} className="block text-base mt-16 mb-2">
        {props.label}
      </label>
      {props.blank && (
        <p className="text-xs text-red-500">Please fill out this section.</p>
      )}
      <select id={props.name} className={`relative w-full ss:w-1/2 p-2 bg-white border-2 rounded-md shadow-sm outline-none focus:border-primary-1 focus:border-2 ${initOption ? 'text-hue-400' : 'text-primary-1'} ${
            props.blank
              ? 'border-red-500'
              : 'border-smoke-2'
